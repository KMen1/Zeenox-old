...
public class SetupCommands : KBotModuleBase
{
    ...
    public async Task SetupTemporaryVoiceChannelsAsync(GuildModules module)
    {
        ...
        foreach (var property in moduleconfig.GetType().GetProperties())
        {
            ...
            if (Context.Guild.GetChannel((ulong)value!) is SocketCategoryChannel category)
            {
                embed.AddField(title, $"`{category.Name}`");
            }
            else if (Context.Guild.GetChannel((ulong)value!) is SocketTextChannel textChannel)
            {
                embed.AddField(title, textChannel.Mention);
            }
            else if (Context.Guild.GetChannel((ulong)value!) is SocketVoiceChannel voiceChannel)
            {
                embed.AddField(title, voiceChannel.Mention);
            }
            else if (Context.Guild.GetRole((ulong) value!) is { } role)
            {
