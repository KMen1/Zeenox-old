...
public class Announcements : SlashModuleBase
{
    ...
    {
        await Mongo
            .UpdateGuildConfigAsync(Context.Guild, x => x.WelcomeChannelId = channel?.Id ?? 0)
    ...
    {
        await Mongo
            .UpdateGuildConfigAsync(Context.Guild, x => x.WelcomeRoleId = role?.Id ?? 0)
            .ConfigureAwait(false);
        var eb = new EmbedBuilder()
    ...
    {
        await Mongo
            .UpdateGuildConfigAsync(Context.Guild, x => x.LeaveChannelId = channel?.Id ?? 0)
            .ConfigureAwait(false);
        var eb = new EmbedBuilder()
            .WithColor(channel is null ? Color.Red : Color.Green)
            .WithDescription(
    ...
    {
        await Mongo
            .UpdateGuildConfigAsync(Context.Guild, x => x.BanChannelId = channel?.Id ?? 0)
            .ConfigureAwait(false);
        var eb = new EmbedBuilder()
            .WithColor(channel is null ? Color.Red : Color.Green)
            .WithDescription(
    ...
    {
        await Mongo
            .UpdateGuildConfigAsync(Context.Guild, x => x.UnbanChannelId = channel?.Id ?? 0)
            .ConfigureAwait(false);
        var eb = new EmbedBuilder()
            .WithColor(channel is null ? Color.Red : Color.Green)
            .WithDescription(
...
public class TemporaryVoice : SlashModuleBase
{
    ...
    public async Task SetCategoryAsync(ICategoryChannel? channel = null)
    {
        await Mongo
            .UpdateGuildConfigAsync(
                Context.Guild,
                x => x.TemporaryVoiceCategoryId = channel?.Id ?? 0
            )
            .ConfigureAwait(false);
        var eb = new EmbedBuilder()
            .WithColor(channel is null ? Color.Red : Color.Green)
    ...
    {
        await Mongo
            .UpdateGuildConfigAsync(Context.Guild, x => x.TemporaryVoiceCreateId = channel?.Id ?? 0)
            .ConfigureAwait(false);
        var eb = new EmbedBuilder()
            .WithColor(channel is null ? Color.Red : Color.Green)
            .WithDescription(
...
public class SetChannelModule : SlashModuleBase
{
    ...
    public async Task SetDbdChannelAsync(ITextChannel? channel = null)
    {
        await Mongo
            .UpdateGuildConfigAsync(
                Context.Guild,
                x => x.DbdNotificationChannelId = channel?.Id ?? 0
            )
            .ConfigureAwait(false);
        var eb = new EmbedBuilder()
            .WithColor(channel is null ? Color.Red : Color.Green)
    ...
    public async Task SetEpicChannelAsync(ITextChannel? channel = null)
    {
        await Mongo
            .UpdateGuildConfigAsync(
                Context.Guild,
                x => x.EpicNotificationChannelId = channel?.Id ?? 0
            )
            .ConfigureAwait(false);
        var eb = new EmbedBuilder()
            .WithColor(channel is null ? Color.Red : Color.Green)
    ...
    {
        await Mongo
            .UpdateGuildConfigAsync(Context.Guild, x => x.LevelUpChannelId = channel?.Id ?? 0)
            .ConfigureAwait(false);
        var eb = new EmbedBuilder()
            .WithColor(channel is null ? Color.Red : Color.Green)
            .WithDescription(
    ...
    {
        await Mongo
            .UpdateGuildConfigAsync(Context.Guild, x => x.AfkChannelId = channel?.Id ?? 0)
            .ConfigureAwait(false);
        var eb = new EmbedBuilder()
            .WithColor(channel is null ? Color.Red : Color.Green)
            .WithDescription(
    ...
    public async Task AddRoleAsync(IRole role, [MinValue(1)] int level)
    {
        await Mongo
            .UpdateGuildConfigAsync(
                Context.Guild,
                x => x.LevelRoles.Add(new LevelRole(role.Id, level))
            )
            .ConfigureAwait(false);
        var eb = new EmbedBuilder()
    ...
    public async Task RemoveRoleAsync(IRole role)
    {
        await Mongo
            .UpdateGuildConfigAsync(
                Context.Guild,
                x => x.LevelRoles.RemoveAll(y => y.Id == role.Id)
            )
            .ConfigureAwait(false);
        var eb = new EmbedBuilder()
            .WithColor(Color.Red)
    ...
    {
        await Mongo
            .UpdateGuildConfigAsync(Context.Guild, x => x.SuggestionChannelId = channel?.Id ?? 0)
            .ConfigureAwait(false);
        await RespondAsync(
                channel is null ? "Suggestions disabled!" : "Channel set!",
                ephemeral: true
            )
            .ConfigureAwait(false);
    }
    ...
    [DefaultMemberPermissions(GuildPermission.ManageGuild)]
    [SlashCommand("setreview", "Set the channel for review messages")]
    public async Task SetReviewChannelAsync(ITextChannel? channel = null)
    {
        await Mongo
            .UpdateGuildConfigAsync(Context.Guild, x => x.ReviewChannelId = channel?.Id ?? 0)
            .ConfigureAwait(false);
        await RespondAsync(channel is null ? "Reviews disabled!" : "Channel set!", ephemeral: true)
            .ConfigureAwait(false);
    }
}
