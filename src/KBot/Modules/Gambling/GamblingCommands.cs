...
public class GamblingCommands : SlashModuleBase
{
    ...
    public async Task SendTransactionsAsync(SocketUser? user = null)
    {
        var transactions = (
            await Mongo
                .GetTransactionsAsync((SocketGuildUser)(user ?? Context.User))
                .ConfigureAwait(false)
        ).ToList();
            ...
            .ConvertAll(
                chunk =>
                    new EmbedBuilder()
                        ...
                        .WithDescription(
                            "To get more information on a transaction use **/gamble transaction <id>**"
                        )
    ...
    public async Task SendTransactionAsync(string id)
    {
        ...
        var embed = new EmbedBuilder()
            ...
            .AddField(
                "Amount",
                $"`{transaction.Amount.ToString("N0", CultureInfo.InvariantCulture)}`",
                true
            )
        ...
        if (transaction.From is not null)
            embed.AddField(
                "From",
                $"`{Context.Guild.GetUser((ulong)transaction.From).Mention}`",
                true
            );
    ...
    )
    {
        ...
        var sourceUser = await Mongo
            .GetUserAsync((SocketGuildUser)Context.User)
            .ConfigureAwait(false);
    ...
    public async Task ClaimDailyCoinsAsync()
    {
        ...
        if (lastDaily == DateTime.MinValue || canClaim)
        {
            ...
            await FollowupWithEmbedAsync(
                    Color.Green,
                    $"Succesfully collected {reward} coins",
                    "",
                    ephemeral: true
                )
                .ConfigureAwait(false);
...
public class AdminCommands : SlashModuleBase
{
    ...
    public async Task ChangeBalanceAsync(SocketGuildUser user, int offset, string reason)
    {
        ...
        var dbUser = await Mongo
            .UpdateUserAsync(user, x => x.Balance += offset)
            .ConfigureAwait(false);
        await FollowupWithEmbedAsync(
                Color.Green,
                "Money set!",
