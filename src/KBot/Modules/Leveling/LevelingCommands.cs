...
public class Levels : KBotModuleBase
{
    ...
    public async Task GetDailyAsync()
    {
        ...
        if (lastDaily == DateTime.MinValue || canClaim)
        {
            ...
            await FollowupWithEmbedAsync(EmbedResult.Success, "Sikeresen begyűjtetted a napi XP-d!", $"A begyűjtött XP mennyisége: {xp.ToString()}", ephemeral: true).ConfigureAwait(false);
        }
        else
        {
    ...
    public async Task SetLevelAsync(SocketUser user, int level)
    {
        ...
        await FollowupWithEmbedAsync(EmbedResult.Success, "Szint beállítva!",
