...
{
    private readonly Dictionary<FilterTypes, Action<PlayerFilterMap>> _filterActions =
        new()
        {
    ...
    public async Task<Embed> DisconnectAsync(IGuild guild, SocketUser user)
    {
        ...
        if (player!.LastRequestedBy.Id != user.Id)
            return new EmbedBuilder()
                .WithColor(Color.Red)
                .WithDescription(
                    "**Only the person who added the currently playing song can control the bot!**"
                )
                .Build();
    ...
    public async Task<Embed> MoveAsync(IGuild guild, IVoiceChannel channel)
    {
        ...
        return new EmbedBuilder()
            .WithDescription($"**Succesfully moved to {channel.Mention}**")
            .WithColor(Color.Green)
            .Build();
    }

    public async Task<Embed?> PlayAsync(
        IGuild guild,
    ...
    )
    {
        ...
        var player = _lavaNode.HasPlayer(guild.Id)
          ...
          : await _lavaNode
                .JoinAsync(
                    () => new MusicPlayer(voiceChannel, message, _youtubeService, _lavaNode),
                    guild.Id,
                    voiceChannel.Id
                )
        ...
        if (player!.IsPlaying)
        {
            ...
            return new AddedToQueueEmbedBuilder(
                new List<LavalinkTrack> { searchResponse.Tracks[0] }
            ).Build();
        }
    ...
    public async Task<Embed?> PlayFromSearchAsync(
        IGuild guild,
        SocketGuildUser user,
        IUserMessage message,
        string id
    ...
    {
        var voiceChannel = user.VoiceChannel;
        var track = await _lavaNode
            .GetTrackAsync("https://www.youtube.com/watch?v=" + id)
            .ConfigureAwait(false);
        track!.Context = user;
        var player = _lavaNode.HasPlayer(guild.Id)
          ? _lavaNode.GetPlayer<MusicPlayer>(guild.Id)
    ...
    public async Task<Embed?> SetVolumeAsync(
        IGuild guild,
        SocketUser user,
        VoiceButtonType buttonType
    )
    {
        var player = _lavaNode.GetPlayer<MusicPlayer>(guild.Id);
        if (player is null)
        ...
        if (
            (currentVolume == 0f && buttonType == VoiceButtonType.VolumeDown)
            || (currentVolume == 1f && buttonType == VoiceButtonType.VolumeUp)
        )
            return new EmbedBuilder()
                .WithColor(Color.Red)
                .WithTitle("Volume must be between 0 and 100!")
                .Build();
        switch (buttonType)
        {
    ...
    public async Task<Embed?> ClearFiltersAsync(IGuild guild, SocketUser user)
    {
        ...
        await player
            .SetFilterAsync(FilterTypes.None, _filterActions[FilterTypes.None])
            .ConfigureAwait(false);
        return null;
    }
