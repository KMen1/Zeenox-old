...
public static class Program
{
    private static Task Main()
    {
        Log.Logger = new LoggerConfiguration().Enrich
            ...
            .WriteTo.Sentry(
                x =>
                {
                    ...
                    x.Dsn =
                        "https://76bb0b430dbf48349ed15b3e5d168cea@o88188.ingest.sentry.io/6201115";
                    x.Debug = false;
        ...
        var host = Host.CreateDefaultBuilder()
            ...
            .ConfigureDiscordHost(
                (context, config) =>
                {
                    ...
                    config.Token = context.Configuration
                        .GetSection("Client")
            ...
            .ConfigureOsuSharp(
                (context, options) =>
                {
                    ...
                    {
                        ClientId = context.Configuration
                            .GetSection("OsuApi")
                            .GetValue<long>("AppId"),
                        ClientSecret = context.Configuration
                            .GetSection("OsuApi")
            ...
            .ConfigureServices(
                (context, services) =>
                {
                    ...
                    services.AddSingleton(
                        x => x.GetService<IMongoClient>()!.GetDatabase(config.MongoDb.Database)
                    );
                    services.AddSingleton<OsuClient>();
                    services.Scan(
                        scan =>
                            scan.FromAssemblyOf<IInjectable>()
        ...
        link.SetPath(
            $"C:\\KBot\\{FileVersionInfo.GetVersionInfo(Assembly.GetExecutingAssembly().Location).FileVersion}\\KBot.exe"
        );
