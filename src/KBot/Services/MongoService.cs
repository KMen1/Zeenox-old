...
public class MongoService : IInjectable
{
    ...
    public MongoService(BotConfig config, IMemoryCache cache, IMongoDatabase database)
    {
        ...
        _transactionCollection = database.GetCollection<Transaction>(
            config.MongoDb.TransactionCollection
        );
        _warnCollection = database.GetCollection<Warn>(config.MongoDb.WarnCollection);
        _brCollection = database.GetCollection<SelfRoleMessage>(
            config.MongoDb.ButtonRoleCollection
        );
    }
            ...
            async x =>
            {
                ...
                return (
                    await _configCollection
                        .FindAsync(y => y.GuildId == guild.Id)
    ...
    {
        return await (
            await _configCollection.FindAsync(x => x.GuildId == guild.Id).ConfigureAwait(false)
        )
            .AnyAsync()
            .ConfigureAwait(false);
    }
    ...
    public async Task<IEnumerable<ulong>> GetDbdNotificationChannelIds()
    {
        return (
            await _configCollection
                .FindAsync(x => x.DbdNotificationChannelId != 0)
                .ConfigureAwait(false)
        )
            .ToList()
    ...
    public async Task<IEnumerable<ulong>> GetEpicNotificationChannelIds()
    {
        return (
            await _configCollection
                .FindAsync(x => x.EpicNotificationChannelId != 0)
                .ConfigureAwait(false)
        )
            .ToList()
    ...
    {
        var message = await _brCollection
            .FindAsync(x => x.MessageId == messageId)
            .ConfigureAwait(false);
        return message.FirstOrDefault();
    }

    public async Task<(bool, SelfRoleMessage?)> UpdateReactionRoleMessageAsync(
        IGuild vGuild,
        ulong messageId,
        Action<SelfRoleMessage> action
    )
    {
        var messages = await _brCollection
            .FindAsync(x => x.GuildId == vGuild.Id && x.MessageId == messageId)
            .ConfigureAwait(false);
        ...
        action(message);
        await _brCollection
            .ReplaceOneAsync(x => x.GuildId == vGuild.Id && x.MessageId == messageId, message)
            .ConfigureAwait(false);
        return (true, message);
    }
    ...
    private async Task<User> AddUserAsync(SocketGuildUser user)
    {
        var users = await _userCollection
            .FindAsync(x => x.UniqueId == user.Id + user.Guild.Id)
            .ConfigureAwait(false);
    ...
    public async ValueTask<User> GetUserAsync(SocketGuildUser vUser)
    {
        return (
                await _userCollection
                    .FindAsync(x => x.UniqueId == vUser.Guild.Id + vUser.Id)
                    .ConfigureAwait(false)
            ).FirstOrDefault() ?? await AddUserAsync(vUser).ConfigureAwait(false);
    }
    ...
    public async Task<User> UpdateUserAsync(SocketGuildUser user, Action<User> action)
    {
        ...
        action(dbUser);
        await _userCollection
            .ReplaceOneAsync(x => x.UniqueId == dbUser.UniqueId, dbUser)
            .ConfigureAwait(false);
        return dbUser;
    }
    ...
    public async Task<IEnumerable<Warn>> GetWarnsAsync(SocketGuildUser user)
    {
        ...
        var tasks = dbUser.WarnIds.ConvertAll(
            warnId =>
                _warnCollection
                    .FindAsync(x => x.Id == warnId)
                    .ContinueWith(
                        async x => warns.Add((await x.ConfigureAwait(false)).FirstOrDefault())
                    )
        );
    ...
    public async Task<bool> RemoveWarnAsync(string warnId)
    {
        ...
        var dbUser = (
            await _userCollection
                .FindAsync(x => x.GuildId == warn.GuildId && x.UserId == warn.GivenToId)
                .ConfigureAwait(false)
        ).FirstOrDefault();
        dbUser.WarnIds.Remove(warnId);
        await _userCollection
            .ReplaceOneAsync(x => x.GuildId == warn.GuildId && x.UserId == warn.GivenToId, dbUser)
            .ConfigureAwait(false);
    ...
    {
        var users = (
            await _userCollection.FindAsync(x => x.GuildId == vGuild.Id).ConfigureAwait(false)
        ).ToList();
    ...
    public async Task UpdateGuildConfigAsync(IGuild guild, Action<GuildConfig> action)
    {
        ...
        action(config);
        await _configCollection
            .ReplaceOneAsync(x => x.GuildId == config.GuildId, config)
            .ConfigureAwait(false);
    ...
    public async Task AddTransactionAsync(Transaction transaction, SocketGuildUser? user)
    {
        ...
        if (user is null)
            return;
        await UpdateUserAsync(user, x => x.TransactionIds.Add(transaction.Id))
            .ConfigureAwait(false);
    }
    ...
    public async Task<IEnumerable<Transaction>> GetTransactionsAsync(SocketGuildUser user)
    {
        ...
        var tasks = dbUser.TransactionIds.ConvertAll(
            transactionId =>
                _transactionCollection
                    .FindAsync(x => x.Id == transactionId)
                    .ContinueWith(
                        async x =>
    ...
    {
        return (
            await _transactionCollection.FindAsync(x => x.Id == id).ConfigureAwait(false)
        ).FirstOrDefault();
    }
}
